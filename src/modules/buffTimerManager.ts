/**
 * 기능 계약 — 채팅 로그 기반 버프 타이머
 *
 * - `chatParser`의 `BUFF_USED`만 실제 버프 시작 입력으로 사용하며, 지속시간·이름·이미지는
 *   `assets/data/buffs.json`을 기준으로 합니다. 같은 버프의 더 오래된 로그를 다시 읽어 현재
 *   타이머를 과거로 되돌리거나 연장해서는 안 됩니다.
 * - `buffTimerEnabled`와 버프별 `buffTimerBuffs[id]`를 모두 적용합니다. 같은 `group`의 실제
 *   버프는 서로 교체하지만 설정 화면의 테스트 버프는 여러 개를 동시에 확인할 수 있게 예외입니다.
 * - 남은 시간은 로그 시각을 기준으로 매초 계산하고, 사용자 임계값과 고정 5초 경고를 각각 한 번만
 *   발생시킵니다. 시각 경고와 소리 경고는 별도 설정이며 소리 알림만 알람 이력에 기록됩니다.
 * - HUD 상태는 game-overlay와 버프 타이머 창에 함께 전송합니다. 소리는 한 창에서만 재생하고,
 *   토스트 표시 위치만 사이드바/독 설정에 따라 나눠 중복 재생을 막습니다.
 * - 절전·잠금 중 지난 임계값은 복귀 후 소리로 몰아서 재생하지 않고 `missed-sleep` 이력으로만
 *   남깁니다. `start()`/`stop()`은 리스너와 타이머를 중복 등록하지 않아야 합니다.
 */
