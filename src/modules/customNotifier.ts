 ...
 * 기능 계약 — 사용자 지정 반복 알림
 *
 * - 사용자 지정 알림 창의 `customAlerts`가 단일 설정 원본입니다. 매일 지정 시각 또는 매시간
 *   지정 분을 기준으로, 각 사전 알림 오프셋만큼 앞당겨 실행합니다.
 * - 알림 ID·오프셋·날짜·분 조합별로 한 번만 실행해 같은 분의 스케줄 재검사에서 중복 재생하지
 *   않습니다. 서로 다른 오프셋은 의도적으로 별도 알림입니다.
 * - 정상 실행은 설정된 소리/음량을 재생하고 게임 최소화 또는 `notifyWhenGameClosed` 조건에서
 *   Windows 알림도 보냅니다.
 * - 절전 중 놓친 시각은 복귀 즉시 소리로 몰아서 재생하지 않고 모험일지 알람 이력에만
 *   `missed-sleep`으로 남깁니다. 실제 실행과 누락 이력은 반드시 `getDueCustomAlertsAt`의
 *   동일한 시간 계산 규칙을 사용해야 합니다.
 */
