 ...
 * 기능 계약 — 필드보스 시간 알림
 *
 * - 출현 시각의 기준 원본은 이 파일의 `BOSS_SCHEDULE`이며, 분 단위 스케줄러가 사용자가
 *   선택한 사전 알림 오프셋을 역산해 알림 대상을 결정합니다.
 * - 전체 스위치(`fieldBossNotifyEnabled`)와 보스별 `enabled`를 모두 통과한 항목만 울립니다.
 *   보스별 소리 파일은 필드보스 설정 창이 소유하고 중앙 설정은 상세값을 중복 저장하지 않습니다.
 * - 같은 날짜·분·보스·오프셋은 한 번만 알립니다. 이 키 구조를 단순화하면 같은 시각에 출현하는
 *   서로 다른 보스나 여러 사전 알림이 누락될 수 있습니다.
 * - 정상 알림은 HUD/사운드 경로로 전달하고, 게임 최소화 또는 `notifyWhenGameClosed` 조건에서는
 *   Windows 알림도 보냅니다. 절전 중 지난 알림은 뒤늦게 재생하지 않고 모험일지 알람 이력에만
 *   `missed-sleep`으로 기록합니다.
 * - 이 분 루프는 숙제 초기화 검사도 깨우므로 필드보스 알림만 보고 제거하면 안 됩니다.
 */
