# Google Drive 동기화 개발 계약

> 이 문서는 TW-Overlay v3.0.0의 개발·검증 기준입니다. 실제 릴리즈 전에는 구현된 클라우드 allowlist와 이 문서의 항목이 자동 검사로 일치해야 합니다. 일반 사용 방법은 `docs/google-drive-sync.md`를 따릅니다.
...
![Google 계정 로그인 버튼 위치](../../docs/screenshot/google-sync-login-guide.svg)
...
![Google Drive 앱 데이터 권한 확인 예시](../../docs/screenshot/google-sync-permission-guide.svg)
...
자동 pull은 매번 공유 meta를 확인하되, 이전에 검증한 설정·숙제 파일과 Drive의 `file ID + version + md5Checksum + modifiedTime + size`가 모두 같고 저장한 원격 revision도 같으면 해당 데이터 파일의 media 다운로드와 동일 로컬 상태의 원자 저장을 생략합니다. 파일 ID나 fingerprint가 달라졌거나 로컬 fingerprint가 없는 기존 설치는 기존과 같은 전체 다운로드·schema/checksum/generation 검증 경로를 사용합니다. 사용자가 직접 누른 불러오기와 데이터 확인도 캐시를 신뢰하지 않고 항상 원격 파일을 다시 검증합니다. 업로드 성공 뒤에는 캐시를 비워 다음 pull에서 새 원격 버전을 다시 검증하며, 응답 유실과 다른 PC의 직후 overwrite는 기존 revision·operation 재확인 절차로 처리합니다.
...
| 채팅 오버레이 탭·필터 | 각 창 선택 탭, 표시할 기본 탭, 커스텀 탭, 키워드, 블랙리스트, NPC/XP/엘소 표시, 사기 닉네임 강조 | `chatOverlayTab`, `chatOverlaySubTab`, `chatOverlaySub2Tab`, `chatOverlayVisibleTabs`, `chatOverlayCustomTabs`, `chatOverlayKeywords`, `chatOverlayBlacklistFilters`, `chatOverlayShowNpcChat`, `chatOverlayShowXpGain`, `chatOverlayShowElsoGain`, `chatOverlayHighlightScamNicknames` |
...
| 창 위치·크기 | 창모드 게임 기준 오프셋 `positions`, 창모드 전체화면 오프셋 `windowedFullscreenPositions`, 사용자 이동 표식 `storedPositionKeys`, Follow OFF용 화면 좌표 `fixedWindowPositions`/`fixedWindowPositionsActive`, `width`, `height`, `opacity`, `xpWidgetPos`, `todaySummaryHudPos`, `buffTimerHudPos`, `abandonedWidgetPos`, `forgeQuestHudPos`, `questHudPos`, `chatOverlayWidth/Height`, `chatOverlaySubWidth/Height`, `chatOverlaySub2Width/Height`, `focusedChatWidth/Height`, `contentsCheckerWidth/Height` | 모니터, DPI, 해상도마다 달라지는 PC 종속 정보 |
...
- 파일을 만든 앱의 동기화 형식이 현재 앱보다 새롭거나 파일 종류가 맞지 않거나 현재 앱이 모르는 항목이 있으면 해당 파일은 적용하지 않고 `현재 버전에서 동기화할 수 없음`으로 표시합니다. 이 PC의 기존 값은 그대로 유지되며, TW-Overlay를 최신 버전으로 업데이트하거나 다른 PC의 앱 버전을 확인한 뒤 다시 시도합니다. 정상인 다른 파일은 계속 확인하고 복원합니다.
- 기존 PC의 백그라운드 pull에서도 한 파일의 검증·적용 실패가 다른 정상 파일 수신을 중단하지 않습니다. 정상 파일 변경은 적용하고 손상 파일은 `invalid`, 현재 버전에서 지원하지 않는 파일은 `incompatible`인 부분 결과로 분리합니다.
