...
테일즈위버의 동일 계정 다중 접속 제한과 집/회사 간 물리적 거리를 고려해, 서로 다른 게임 계정으로 동일 숙제를 동시에 플레이하는 시나리오는 필수 실기 게이트에서 제외하고 자동 회귀 검사로 정책을 고정한다.

- 캐릭터 선택 전 같은 숙제·같은 리셋 주기의 pending 충돌은 결정적 operation 순서의 마지막 값으로 수렴한다.
- 캐릭터를 자동으로 추측하거나 같은 감지를 여러 캐릭터에 적용하지 않는다.
- 각 PC에서 팝업의 서로 다른 캐릭터를 선택한 뒤에는 두 캐릭터의 완료 상태와 두 PC의 operation을 모두 보존한다.
- 완료/해제와 서로 다른 횟수 같은 동일 필드 충돌도 operation 입력 순서와 무관하게 양쪽 로컬·원격이 같은 값으로 수렴한다.
- 실제 실기에서는 양쪽 앱을 계속 실행한 상태에서 게임 실행 중 약 30초, 게임 미실행 시 약 5분 안에 순차 변경이 pull되는지만 확인한다.

렌더러 자동 검증에서는 실제 preload가 연결된 체크리스트 창에 메인 프로세스가 `config-data`로 원격 pending fixture를 전송한다. 첫 수신과 `나중에 하기` 이후 재수신에서 캐릭터 선택 팝업이 표시되고, 숙제명·횟수와 두 캐릭터가 렌더링되는지 확인한다. 캐릭터 선택은 올바른 `contents-apply-pending` ID를 보내고, 보류 내역 삭제는 확인 뒤 `contents-clear-pending`을 보내며 두 경우 모두 팝업을 닫는다. 캐릭터가 한 명이거나 pending이 비어 있으면 팝업을 표시하지 않는다.
...
- [ ] 버프 타이머·경험치·어벤던·퀘스트 HUD 위치를 각각 저장한 뒤 캐릭터 재접속과 게임 해상도 전환을 반복한다. 게임 화면이 잠시 축소돼도 저장 좌표가 중앙으로 바뀌지 않고 원래 위치로 돌아오는지 확인한다.
...
- [ ] 경험치 세션을 정지한 상태에서 양수 경험치 로그를 누적해도 세션 총 경험치·처치 수는 증가하지 않고 경고용 진행도만 증가하는지 확인한다.
- [ ] 교환 없이 110억에 도달하면 한 번, 같은 구간에서는 반복 없이 210억에 도달할 때 다시 한 번 경고하는지 확인한다.
- [ ] `<font size="2" color="#ff64ff">경험치가 10000000000 감소했습니다.</font>` 로그가 들어오면 경고용 진행도는 0이 되고, 세션 측정 중이면 정수 교환 횟수만 1 증가하며 세션 총 경험치는 줄지 않는지 확인한다.
- [ ] 세션 초기화는 총 경험치·처치 수·세션 정수 횟수만 0으로 만들고 경고용 진행도는 유지하는지 확인한다.
...
- [ ] 업데이트 공지와 작업표시줄에 표시되는 기능 창을 각각 연다. 시작 메뉴뿐 아니라 실행 중 작업표시줄에도 Electron 기본 원자 아이콘 대신 TW-Overlay 아이콘이 표시되는지 확인한다.
- [ ] 독 메뉴 클릭, 독 숨김/표시, 같은 모니터 Alt+Tab, 다른 모니터 Alt+Tab을 `ef4cfde` 이후 다시 조합한다.
- [ ] 브라우저·카카오톡을 foreground로 둔 채 사이드바 1depth를 hover하고 열린 서브메뉴 항목을 한 번 클릭한다. 첫 클릭으로 기능 창이 열리고 같은 입력이 두 번 실행되지 않는지 확인한다.
- [ ] 게임을 최소화하지 않고 뒤에 둔 상태에서 브라우저·카카오톡을 클릭해도 테일즈위버와 TW-Overlay가 함께 앞으로 튀어나오지 않는지 확인한다.
- [ ] 사용자가 테일즈위버로 돌아오면 모든 가시 TW-Overlay 창이 기존 내부 순서대로 게임 위에 복귀하고, 포커스는 게임에 남는지 확인한다.
- [ ] 게임과 같은 모니터에서 외부 A를 선택한 뒤 외부 B를 선택한다. A/B의 Windows Z-order가 유지되고 둘 다 게임과 모든 TW-Overlay 창보다 위에 있는지 창모드와 창모드 전체화면에서 각각 확인한다.
- [ ] B를 다른 모니터로 드래그한다. 게임 HUD·오늘의 요약·경험치 HUD가 외부 A를 뚫고 올라오지 않고 `외부 A/B > TW-Overlay > 게임` 관계를 유지하는지 두 모드에서 각각 확인한다.
- [ ] 다른 모니터의 외부 C를 직접 선택해도 같은 관계가 유지되고, C foreground를 빼앗거나 게임이 함께 앞으로 오지 않는지 확인한다.
- [ ] 외부 A가 최소화되지 않은 게임과 TW-Overlay를 완전히 가린 상태에서 작업표시줄의 TW-Overlay 창을 선택한다. 선택한 창과 게임이 함께 A 위로 올라오고, 최종 포커스는 선택한 TW-Overlay 창인지 두 모드에서 확인한다.
- [ ] 같은 조건에서 게임을 사용자가 직접 최소화한 뒤 작업표시줄의 TW-Overlay 창을 선택한다. 최소화된 게임은 복원되지 않고 선택한 TW-Overlay 창만 열리는지 확인한다.
- [ ] 채팅 오버레이와 숙제 체크리스트를 다른 모니터로 옮긴 뒤 그 모니터의 브라우저·카카오톡을 클릭한다. 클릭한 외부 앱이 두 창보다 위에 표시되는지 두 모드에서 각각 확인한다.
- [ ] 게임창 따라가기를 켠 상태에서 채팅·숙제·도구 창을 옮기고 게임창 이동·게임 재실행·앱 재시작을 반복한다. 각 창이 저장한 게임 기준 간격을 유지하는지 확인한다.
- [ ] 게임창 따라가기를 끈 상태에서 같은 창을 옮기고 게임창 이동·게임 재실행·앱 재시작을 반복한다. 각 창이 같은 화면 절대 위치를 유지하며, ON→OFF→ON→OFF 전환 때 과거 위치로 튀지 않는지 확인한다. 게임 HUD·사이드바·독은 계속 게임을 따라야 한다.
- [ ] 게임과 TW-Overlay를 켠 채 `창모드 → 창모드 전체화면 → 창모드`를 반복한다. 전환 중 채팅·숙제·도구 창이 화면 밖이나 중앙으로 튀지 않고, 완료 뒤 각 화면 모드에서 마지막으로 옮긴 위치가 독립 복원되는지 확인한다. 게임 HUD·사이드바·독은 전환 중에도 게임 경계를 따라야 한다.
- [ ] 게임을 최소화하거나 종료한 뒤 트레이에서 채팅·숙제·도구 창을 연다. 창이 현재 화면 중앙에 열리고, 이 상태에서 창을 옮긴 뒤 게임을 복원해도 게임 실행 중 저장해 둔 창모드별 위치가 덮이지 않는지 확인한다.
- [ ] 테일즈위버를 먼저 실행하고 게임이 활성화된 상태에서 TW-Overlay를 실행한다. 앱 시작 직후 단축키가 첫 입력부터 동작하는지 확인하고, 반대 순서(앱 먼저, 게임 나중)와 외부 앱 활성 상태의 단축키 미동작도 함께 확인한다.
- [ ] 위 각 상태에서 게임으로 돌아오면 모든 가시 TW-Overlay 창이 기존 내부 순서대로 게임 바로 위에 복구되고 게임 foreground를 빼앗지 않는지 확인한다.
- [ ] 작업표시줄은 TW-Overlay를 끈 Windows 기준 동작과 같아야 한다. TW-Overlay가 Shell을 강제로 표시·숨김 처리하거나 게임 포커스를 되찾아 기준 동작을 바꾸지 않는지 확인한다.
- [ ] 브라우저·카카오톡이 전경인 동안 불꽃·젤리삐 HUD 효과가 발생해도 외부 앱 위로 표시되지 않는지 확인한다.
- [ ] HUD 편집 모드·사냥 동선 오버레이·업데이트 공지를 열어 둔 채 외부 앱으로 전환하면 전체 TW-Overlay 묶음이 게임과 함께 자연스럽게 뒤로 가는지 확인한다.
- [ ] 자동 생성되는 브라우저·채팅·숙제·독 창이 게임 포커스를 빼앗지 않는지 확인한다.
- [ ] 독과 입력 도구 창은 사용자가 명시적으로 클릭했을 때 정상 입력 가능한지 확인한다.
- [ ] 독 패널과 열린 플라이아웃은 정상 클릭되고, 독 창의 투명 여백에서는 게임 클릭·마우스 이동이 통과하는지 확인한다.
...
자동 검증 기록(2026-08-26, `ef4cfde`, `d562c17`, `452f9c3`): `v2.7.1` 실사용 피드백으로 게임을 포함한 동적 Topmost 묶음을 폐기했다. 이후 실제 HWND probe에서 일반 Electron 오버레이에 `HWND_TOP`만 적용하면 foreground 게임 아래에 남는 결함과 숨은 Electron 보조 HWND가 보이는 순서 검사를 깨는 현상을 재현했다. 게임·TW-Overlay 전경에서는 우리 창만 임시 Topmost로 올리고, 외부 전경에서는 즉시 Non-Topmost로 내리도록 보강했다. 게임 HWND 쓰기 0회, 외부 다중 창 순서 보존, 반복 판정 멱등성과 숨은 HWND 건너뛰기를 회귀 테스트로 고정했다. 실행법은 [`fake-talesweaver-fixture.md`](fake-talesweaver-fixture.md)에 기록했다.

자동·부분 실기 기록(2026-08-26, `8cd1d2e`, `298813e`): 다른 모니터 외부 전경에서 전체 TW-Overlay를 Topmost로 유지한 최초 보정은 채팅·숙제 창이 외부 앱을 가렸고, 창별 rect 분기도 외부 B 이동 뒤 게임 HUD가 기존 외부 A를 뚫고 올라오는 회귀를 남겼다. 전부 Non-Topmost로 고정한 시도는 실게임에서 게임 선택 즉시 모든 우리 창이 뒤로 숨는 것도 확인했다. `298813e`는 게임·TW-Overlay 전경에서만 우리 창을 임시 Topmost로 올리고 외부 전경에서는 모니터·겹침과 무관하게 모두 게임 band로 내린다. 강등 전 보이는 외부 anchor를 읽어 `게임 < TW-Overlay < 기존 외부 창`을 복원하며 게임·외부 HWND에는 쓰지 않는다. 이 정책은 변경 금지 주석과 창모드·창모드 전체화면 자동 회귀로 고정했고 전체 필수 게이트를 통과했다. 관리자 PowerShell의 실제 HWND probe도 두 모드에서 게임·외부 foreground 보존, 게임 bounds·Topmost 불변, 게임 활성 오버레이 승격, 외부 활성 강등, 외부 우선과 스택 복구를 모두 통과했다. 사용자가 보고한 A→B→B를 다른 모니터로 이동하는 실게임 시나리오는 현재 실행 모드에서 정상임을 확인했다. 실제 게임의 두 모드 전체 조합과 장시간 소크는 대기로 둔다.
...
- [ ] 과거 로그 동기화 중 잠긴 일부 파일이 부분 실패로 표시되고 정상 파일 결과는 반영되는지 확인한다.
- [ ] 모험일지 보관 기간만큼 실제 대량 ChatLog가 있는 환경에서 설정 창이 멈추지 않고 준비·분석 진행률과 실패 파일을 계속 표시하는지 확인한다.
...
| Z-order 소크 | `1291779`, `ef4cfde`, `d562c17`, `452f9c3`, `8cd1d2e`, `298813e` | 사용자 실게임 이력 / 단위 회귀 / 관리자 가짜 게임 실제 HWND probe | 2026-08-25~26 | 부분 통과 | 기존 독 재사용·작업표시줄 복구 이력 보존. 게임 전경에서는 우리 창만 임시 Topmost로 올리고, 외부 전경에서는 모니터와 무관하게 전부 게임 band로 내려 `게임 < TW-Overlay < 기존 외부 창`으로 복원한다. 창모드·창모드 전체화면 격리 정책, 전체 필수 게이트, 관리자 probe 통과. 보고된 A/B/다른 모니터 이동 실게임 시나리오 정상 확인. 실제 게임 두 모드 전체 조합·30~60분 소크 대기 |
