...
![Version](https://img.shields.io/badge/version-3.1.0-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v3.1.0 (2026.08.30)
이번 **v3.1.0 업데이트**는 QTE 챌린지와 발굴지 포탈 현황판을 추가하고, 진화 재료 비용 계산기와 버프 백과의 사용 흐름을 전면 개선한 기능 업데이트입니다.

- **🎯 QTE 챌린지 추가:** 테일즈위버 QTE의 한 바퀴 판정, 일반 성공·대성공 영역과 속도 증가를 게임처럼 연습할 수 있습니다.
- **🗺️ 발굴지 포탈 HUD 추가:** 방문하지 않은 포탈을 실제 맵 배치에 맞춰 표시하고, 설정에서 사용 여부와 위치를 관리할 수 있습니다.
- **🧮 진화 재료 비용 계산 개선:** 장비 부위·진화 구간·베이스 확보 방식·후처리 비용을 나눠 계산하고 저장 이력을 수정하거나 다시 불러올 수 있습니다.
- **⚡ 버프 백과 개편:** 버프를 가나다순으로 찾고 직접 조합하거나, 빈 프리셋과 현재 선택 기반 프리셋을 구분해 생성·수정·삭제할 수 있습니다.
- **🛍️ Microsoft Store 업데이트 개선:** Store 설치본도 자동 업데이트 설정과 필수 업데이트 여부에 따라 시작 화면에서 업데이트하거나 알림만 표시합니다.
...
- **발굴지 포탈 현황판**: 발굴지 진입 뒤 미방문 포탈을 실제 맵 배치에 맞춰 게임 HUD에 표시
...
- **[QTE 챌린지](./docs/qte-challenge.md)**: 한 바퀴 안에 일반 성공·대성공 영역을 노리는 테일즈위버 QTE 연습 게임
- **[마정석 가치 계산기](./docs/magic-stone-calculator.md)**: 획득한 마정석 수량별 수익 정산 도구
- **[진화 재료 비용 계산기](./docs/evolution-calculator.md)**: 장비 진화 구간별 재료, 베이스 장비, 인챈트와 후처리 비용 계산
...
### 설치 방법
[Releases](https://github.com/twliker/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-3.1.0.exe` 파일을 다운로드하여 실행하세요.
