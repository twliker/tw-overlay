...
![Version](https://img.shields.io/badge/version-3.0.2-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v3.0.2 (2026.08.29)
이번 **v3.0.2 업데이트**는 v3.0.1 배포 후 진행한 소스 감사 결과를 반영해 데이터 복구·클라우드 동기화·외부 파일 검증을 강화하고, 누락된 게임 완료 알림 설정을 보완한 안정화 패치입니다.

- **☁️ 설정·숙제 보호 강화:** 원격 변경, 재로그인, 설정만 선택 복원과 앱 재시작이 겹쳐도 로컬 변경과 숙제 상태를 보존합니다.
- **🛡️ 백업·AI 파일 안전성 강화:** 백업 복원 범위를 검증하고 AI 실행 파일·모델을 확인한 뒤 중단에도 복구 가능한 방식으로 교체합니다.
- **🔔 게임 완료 알림 분리:** 경험의 정수, 도전과제, 심연의 보물창고의 표시 여부·알림음·음량을 각각 설정할 수 있습니다.
- **📋 숙제 정렬과 설정 UX 개선:** 일일은 일일 안에서, 주간은 주간 안에서만 이동하며 순서가 재시작 뒤에도 유지됩니다. 설정 안내와 키보드 조작도 보완했습니다.
- **📖 과거 기록 복원 강화:** 완료된 로그 전체 재분석, 경험의 정수·마정석 집계, 지난주 숙제와 주간 상세 완료 수를 정확하게 복원합니다.
- **🪟 Windows 창 동작 수정:** 외부 앱 사용 중 사이드바 서브메뉴 첫 클릭과 실행 중 작업표시줄 제품 아이콘을 바로잡고, 창모드↔창모드 전체화면 전환 때 보조 창 위치가 초기화되지 않도록 각 모드의 위치를 따로 기억합니다.
...
### 설치 방법
[Releases](https://github.com/twliker/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-3.0.2.exe` 파일을 다운로드하여 실행하세요.
