...
![Version](https://img.shields.io/badge/version-2.7.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.7.1 (2026.08.25)
이번 **v2.7.1 안정화 업데이트**는 2.7.0에서 보고된 오버레이 표시, 독 조작, 숙제 및 모험 일지 데이터 보존 문제를 집중적으로 수정한 패치 버전입니다.

- **🪟 오버레이·독 안정화:** 게임과 다른 프로그램을 전환하거나 모니터 환경이 바뀌어도 오버레이가 게임 뒤로 숨거나 작업표시줄에 고착되는 문제를 수정했습니다.
- **💾 설정·숙제 데이터 보호:** 기존 사용자 설정, 숙제 표시 상태와 완료 횟수를 안전하게 보존하며 초기화·병합 과정에서 값이 되돌아가는 문제를 보완했습니다.
- **📖 모험 일지 복구 강화:** 저장 실패와 비정상 종료 상황에서 기록 손실을 줄이고, 큰 금액 및 수량이 정확히 유지되도록 수정했습니다.
- **⏰ 알림·업데이트 안정성 개선:** 절전 복귀 후 놓친 알림 처리와 여러 버전을 건너뛰는 업데이트 흐름을 안정화했습니다.
...
### 설치 방법
[Releases](https://github.com/twliker/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-2.7.1.exe` 파일을 다운로드하여 실행하세요.
