...
![Version](https://img.shields.io/badge/version-3.1.4-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v3.1.4 (2026.09.12)
이번 **v3.1.4 업데이트**는 설정과 백업·동기화를 안정화하고, 숙제 기록·거래 알림·장비 계산기의 오류를 수정한 업데이트입니다.

- **🪟 Store 자동 실행 수정:** Windows 로그인 시 발생하던 경로 접근 오류를 수정했습니다. 업데이트 후 앱을 한 번 실행하면 이전의 잘못된 Store 자동 실행 등록을 정리합니다.
- **⚙️ 설정 편집 내용 보존:** 다른 설정이 갱신되거나 저장에 실패해도 작성 중인 내용을 유지하고, 저장 성공 여부를 확인한 뒤 설정 창을 닫습니다.
- **💾 백업 범위 확대:** ZIP 백업에 버프 프리셋, 계수 계산기 프로필, 진화 계산기 입력·기록, QTE 기록 등 보조 도구의 저장 데이터도 포함합니다.
- **☁️ 동기화 안정화:** 여러 PC의 변경이 겹칠 때 다시 확인해 병합하고, 동기화된 설정과 숙제 순서를 실행 중인 앱에도 반영합니다.
- **✅ 숙제·알림 오류 수정:** 숙제 횟수와 초기화 규칙 변경 시 완료 기록을 바로잡고, 거래 알림의 재시작 복구와 검색별 오류 처리를 개선했습니다.
- **🧮 계산기 정확도 개선:** 인챈트 축복과 비아누 성공 횟수를 계산에 반영하고, 인크립트 목표 입력 범위를 보정합니다.

자세한 내용은 [v3.1.4 릴리즈 노트](release-note/CHANGELOG-v3.1.4.md)를 확인해 주세요.
...
### 설치 방법
[Releases](https://github.com/twliker/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-3.1.4.exe` 파일을 다운로드하여 실행하세요.
