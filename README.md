...
![Version](https://img.shields.io/badge/version-3.0.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v3.0.1 (2026.08.28)
이번 **v3.0.1 업데이트**는 v3.0.0 배포 후 확인된 창 전환과 표시 회귀를 수정하고, 모험 일지와 과거 채팅 로그 복원의 정합성·사용성을 보완한 패치입니다.

- **🪟 작업표시줄 창 전환 보완:** 다른 프로그램이 게임을 가린 상태에서 작업표시줄의 TW-Overlay를 선택하면 게임과 TW-Overlay가 같은 작업 묶음처럼 함께 표시됩니다.
- **📖 모험 일지 개선:** 등록 해제한 자동 득템을 숨기고, 마정석을 일반 득템 개수에서 제외하며, 주간·월간 득템 목록과 날짜별 합산 표시를 제공합니다.
- **💬 과거 로그 복원 개선:** 모험 일지 보관 기간의 채팅 로그를 분석하고 오늘 기록을 안전하게 재구성하며, 진행률과 실패 파일을 계속 안내합니다.
- **🔔 알림·HUD 회귀 수정:** 커스텀 알림음 파일 관리 메뉴를 다시 독립적으로 표시하고 `Ctrl + Shift + Z`의 경험치 세션 일시정지·HUD 숨김 동작을 복원했습니다.
...
### 설치 방법
[Releases](https://github.com/twliker/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-3.0.1.exe` 파일을 다운로드하여 실행하세요.
