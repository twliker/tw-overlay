...
![Version](https://img.shields.io/badge/version-3.1.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v3.1.1 (2026.08.30)
이번 **v3.1.1 업데이트**는 모험일지의 득템 집계를 더 직관적으로 개선하고, 장사 수익·더블 리워드 기록과 HUD 위치 편집 오류를 수정한 안정화 업데이트입니다.

- **📦 득템 품목별 합계 추가:** 선택한 주간·월간 기간에 어떤 아이템을 총 몇 개 획득했는지 한눈에 확인할 수 있습니다.
- **↕️ 득템 기록 영역 조절:** 품목별 합계와 일자별 기록 사이의 구분선을 드래그해 원하는 높이로 조절할 수 있으며 재실행 후에도 유지됩니다.
- **💰 수익·보상 기록 수정:** 직접 입력한 장사 수익이 SEED 통계에 반영되고, 더블 리워드 추가 보상도 실시간·과거 로그에서 감지됩니다.
- **🧭 HUD 위치 저장 안정화:** 위치 편집 중 일반 설정 저장으로 HUD가 사라지던 문제를 막고, 기존에 손상된 위치는 업데이트 후 한 번 복구합니다.
...
### 설치 방법
[Releases](https://github.com/twliker/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-3.1.1.exe` 파일을 다운로드하여 실행하세요.
