...
- 인챈트의 현재 축복치는 첫 성공까지의 기댓값에 반영합니다. 목표가 여러 번이면 첫 성공 뒤 축복치가 초기화되는 조건으로 나머지 비용을 더합니다.
- 비아누 인크립트는 현재 인크립트 횟수에서 시작해 성공할 때마다 달라지는 확률로 다음 성공의 기댓값을 계산합니다. 목표 횟수의 총비용은 각 단계의 예상 소모량을 합한 값입니다.
- 인크립트 목표 성공 횟수는 1~20회입니다. 범위를 벗어난 직접 입력이나 붙여넣기는 계산 전에 보정하며, 소수는 정수 부분을 사용합니다.
