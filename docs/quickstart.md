...
- 사이드바·독의 마우스 아이콘 또는 기본 단축키 `Ctrl + Shift + T`로 마우스 투과를 전환합니다. 아이콘이 초록색이면 웹 브라우저 오버레이와 채팅 오버레이(메인·보조 1·보조 2)의 클릭·휠·드래그가 게임으로 전달됩니다. 사이드바·독과 설정 창은 투과 대상이 아닙니다.
...
- **오버레이를 클릭하거나 스크롤할 수 없음:** 사이드바·독에서 초록색 마우스 아이콘을 누르거나 `Ctrl + Shift + T`로 마우스 투과를 해제하세요.
- **알림 소리는 들리지만 사이드바·채팅 오버레이가 보이지 않음:** 채팅 로그 인식과 게임 창 감지는 별개입니다. 게임 최소화 여부와 두 프로그램의 관리자 권한을 확인하세요.
