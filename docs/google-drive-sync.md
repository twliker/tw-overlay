...
4. 현재 `불러오기`는 두 파일을 함께 확인해 일반 설정에는 클라우드 스냅샷을 적용하고 숙제에는 3방향 병합을 적용합니다. 파일별 선택 복원 UI는 릴리즈 전 추가 검증 항목입니다.
5. 현재 데이터 확인 화면은 두 파일을 검증한 뒤 합친 읽기 전용 미리보기를 제공합니다. 파일별 revision·대기 변경 표시 UI는 릴리즈 전 추가 검증 항목입니다.
...
| 동기화 제어 상태 | payload revision/checksum과 최근 확인 operation ID. 마지막 정상 동기화 기준본과 미전송 outbox 자체는 각 PC의 로컬 `cloud-sync-state.json`에만 저장 |
