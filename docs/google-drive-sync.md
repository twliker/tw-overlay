...
설정은 변경 후 약 1.5초, 숙제는 변경 후 약 0.5초 동안 변경을 모아 해당 파일만 업로드합니다. 변경할 일이 없으면 데이터 파일을 반복 업로드하지 않습니다. 다른 PC의 변경 확인은 게임 실행 중 약 30초, 게임 미실행 시 약 5분 간격이며 앱·게임 시작, 절전 복귀와 네트워크 복구 때는 바로 확인합니다.
