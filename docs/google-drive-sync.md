...
5. 데이터 확인 화면은 두 파일을 각각 검증한 뒤 정상 파일만 합친 읽기 전용 미리보기를 제공합니다. 파일이 없거나 손상됐거나 생성 세대가 다르면 해당 파일의 상태를 따로 표시하며 정상인 다른 파일은 계속 확인할 수 있습니다.
...
- `fresh` 복원은 메타 파일이 가리키는 파일 ID를 우선 사용합니다. 중복 파일은 checksum·종류·생성 세대 검증을 통과한 후보만 사용하며, 최신 메타가 손상됐으면 다음 유효 메타 후보를 확인합니다.
...
현재 구현은 파일별 선택 복원, 검증 결과, 부분 복원 상태와 합친 JSON 미리보기를 제공합니다. 로컬/클라우드 revision·dirty·재시도 상태, 필드별 변경 요약과 백업 되돌리기 진입점은 릴리즈 전 남은 UI 작업입니다.
