...
자동 강화 기록(2026-08-26, `8f643f1`, `9974c5a`, `ab443e9`, `2fe2491`, `5043162`): 완료·해제·횟수 0~3, 같은 필드 충돌·다른 캐릭터 변경과 operation 순서를 조합한 256개 결정론적 stress fixture에서 원격 결과와 두 로컬 상태가 모두 수렴하고 최종 payload에 두 operation ID가 유지됐다. 이어 독립 userData의 실제 `main.js` 두 프로세스를 동시에 실행해 같은 원격 revision을 읽게 한 뒤 최초 두 업로드의 상호 overwrite와 총 3회 이상의 누락 operation 재게시를 강제했으며, 최종 원격·양쪽 로컬 확인 이력과 숙제 상태가 모두 수렴하고 outbox가 비워졌다. 같은 숙제·캐릭터의 기준 기록을 비워 회사의 완료와 집의 더 늦은 미완료·횟수 2를 모두 실제 mutation으로 만든 충돌에서도 최종 미완료·횟수 2와 두 operation ID가 원격·양쪽 로컬에 수렴했다. 수렴한 양쪽 userData를 각각 새 메인 프로세스로 재실행해도 operation·숙제 상태가 유지되고 추가 echo upload는 발생하지 않았다. Windows 테스트 잠금 디렉터리 경합에서 발생한 일시 `EPERM`은 fixture의 제한 재시도로 보강했다. 실제 두 Windows PC와 Google Drive의 전송 시각·poll 간격·echo 여부를 대신하지 않으므로 실기 항목은 대기로 둔다.
...
|---|---|---|---|---|---|
| 두 PC 클라우드 | `8f643f1`, `9974c5a`, `ab443e9`, `2fe2491`, `5043162` | 독립 userData 실제 `main.js` 2개 / 지속형 모의 Drive | 2026-08-26 07:48~08:27 KST | 부분 통과 | 같은 revision 교차 업로드·overwrite·누락 operation 재게시, 비충돌 변경과 완료/미완료·횟수 동일 필드 충돌의 최종 원격/양쪽 로컬 두 operation 보존·outbox 정리, 양쪽 재시작 후 무 echo 확인. 실제 Google 계정·두 Windows PC·poll/echo 대기 |
