...
부분 계측 기록(2026-08-26, `be3c13b`): 실제 Electron 격리 프로세스에서 40MB 당일 로그에 4MB를 추가해 최근 16MB 제한 모드, 오래된 4,317줄 trim, 재시작 뒤 제한 모드와 최근 marker 보존을 확인했다. 10초 동안 1,200개 이벤트를 처리한 지속 부하에서는 초당 119줄, 최대 event-loop lag 1ms, heap 증가 0.55MB였고 채팅 DOM 43개·포커스 DOM 150개 상한을 유지했다. 별도 Tail 검사에서는 오류 직후 파일을 일시 이동해 1초 재시도를 실패시킨 뒤 2초 후속 예약과 복원 재연결을 확인했으며, 기존 marker와 신규 live marker가 각각 정확히 1건이었다. 실제 게임 로그 장시간 소크, OS 파일 잠금, UTF-8/EUC-KR/BOM 전체 조합은 수행하지 않았으므로 위 체크 항목은 계속 대기로 둔다.
...
| 대형 로그 소크 | `be3c13b` | 격리 Electron / 합성 UTF-8 | 2026-08-26 | 부분 통과 | 40→44MB 제한·trim·재시작, 10초/1,200 이벤트, Tail 파일 부재→복원 확인. 장시간·실제 잠금·인코딩 조합 대기 |
