...
주기 계측 기록(2026-08-26, `fb49591`, `055dfbd`, `54f9320`): 실제 `cloudSyncManager` scheduler는 같은 installation ID의 jitter를 적용해 게임 실행 상태에서 27~33초, 미실행 상태에서 270~330초 범위의 다음 pull을 각각 하나 예약하고 중지 시 정리했다. 게임 실행 중 Drive 목록 조회 실패 뒤 첫 재시도는 54~66초 범위로 증가했고, 다음 성공 뒤 27~33초 범위로 초기화됐다. 즉시 pull은 기존 장기 타이머를 취소하고 Drive 목록을 바로 조회한 뒤 정상 후속 타이머 하나를 다시 예약했다. 후속 dirty 업로드 정리에는 별도 목록 조회가 추가될 수 있다. 실제 두 PC·Google Drive에서 변경이 해당 시간 안에 도착하고 OS 이벤트가 즉시 요청을 발생시키는지는 실기로 남긴다.
