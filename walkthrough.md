...
- Z-order는 중앙 상태 관리자가 전담하고, 독은 숨긴 창을 재사용한다. 게임·TW-Overlay가 전경이면 우리 창만 임시 Topmost로 올리고 외부 전경에서는 즉시 내리며, 게임 HWND와 외부 창은 읽기만 한다.
...
`1291779`은 사용자 실게임·듀얼 모니터 환경에서 독 클릭·숨김/표시, 작업표시줄 복구, 게임 위 오버레이 유지, 숨긴 독의 위치·퀵링크 즉시 반영을 확인했다. 하지만 `v2.7.1` 사용자 피드백으로 외부 앱 클릭 시 게임까지 같이 재등장하는 회귀를 확인했다. `ef4cfde`에서 게임 HWND를 쓰기 대상에서 제외하고 `d562c17`에서 모든 game-attached 창을 중앙 정책으로 통합했다. `452f9c3`의 실제 HWND 검사에서는 일반 Electron 창을 Non-Topmost 맨 위로 옮기는 것만으로 foreground 게임 위에 표시되지 않는 결함을 재현했다. 게임·TW-Overlay 전경에서는 우리 창만 임시 Topmost로 올리고 외부 전경에서는 즉시 내리며, 숨은 Electron 보조 HWND를 보이는 창 순서에서 제외하도록 수정했다. 실제 tracker가 우회 없이 탐지하는 관리자 기본 가짜 테일즈위버로 창모드·창모드 전체화면을 실행한 일반 권한 개발 probe에서 게임 위치·크기·Topmost·foreground 불변, 오버레이 복구와 외부 창 우선을 확인했다. 관리자 동일 권한, 실게임·듀얼 모니터, 자동 생성 창·입력 도구 전체와 30~60분 무조작 소크는 대기다.
