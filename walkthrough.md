...
`1291779`은 사용자 실게임·듀얼 모니터 환경에서 독 클릭·숨김/표시, 작업표시줄 복구, 게임 위 오버레이 유지, 숨긴 독의 위치·퀵링크 즉시 반영을 확인했다. 하지만 `v2.7.1` 사용자 피드백으로 외부 앱 클릭 시 게임까지 같이 재등장하는 회귀를 확인했다. `ef4cfde`에서 게임 HWND를 쓰기 대상에서 제외하고 모든 가시 TW-Overlay 창만 기존 내부 순서로 게임 바로 위에 놓도록 수정했다. `d562c17`에서 HUD 효과·HUD 편집·사냥 동선 오버레이와 업데이트 공지도 중앙 정책으로 통합했다. 외부 다중 창 순서, 일반 게임+Topmost Shell, Topmost 게임, 멱등성, 중앙 정책 밖 game-attached Topmost 부재 자동 검증은 통과했다. 새 정책의 실게임·듀얼 모니터·창모드 전체화면 재검증과 자동 생성 창·입력 도구 전체·30~60분 무조작 알림 소크는 대기다.
