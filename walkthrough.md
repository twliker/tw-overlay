...
- Z-order는 중앙 상태 관리자가 전담하고, 독은 숨긴 창을 재사용한다. 게임·TW-Overlay가 전경인 동안에는 우리 창만 임시 Topmost로 올린다. 외부 창이 전경을 얻으면 모니터 위치와 화면 겹침에 관계없이 모든 우리 창을 게임 band로 내리고 기존 내부 순서대로 게임 바로 위·기존 외부 창 아래에 둔다. 게임 HWND와 외부 창은 읽기만 한다.
...
`1291779`은 사용자 실게임·듀얼 모니터 환경에서 독 클릭·숨김/표시, 작업표시줄 복구, 게임 위 오버레이 유지, 숨긴 독의 위치·퀵링크 즉시 반영을 확인했다. 하지만 `v2.7.1` 사용자 피드백으로 외부 앱 클릭 시 게임까지 같이 재등장하는 회귀를 확인했다. `ef4cfde`에서 게임 HWND를 쓰기 대상에서 제외하고 `d562c17`에서 모든 game-attached 창을 중앙 정책으로 통합했다. `8cd1d2e`와 후속 창별 rect 분기는 다른 모니터 전환 시 일부 우리 창을 Topmost로 남겨, 채팅·숙제 창이 외부 앱을 가리거나 게임 HUD가 기존 외부 A를 뚫고 올라오는 회귀가 있었다. 반대로 전부 Non-Topmost로 고정한 시도는 실게임에서 게임 선택 즉시 모든 우리 창이 뒤로 숨었다. `298813e`는 게임·TW-Overlay 전경에서만 우리 창을 임시 Topmost로 올리고, 외부 전경에서는 모니터와 무관하게 전부 게임 band로 내린다. 강등 전에 게임에서 위로 탐색해 처음 만난 보이는 외부 HWND를 기억하고 그 아래에 우리 묶음만 삽입해 `게임 < TW-Overlay < 기존 외부 창`을 복원한다. 숨은 Electron 보조 HWND는 보이는 창 순서에서 제외하고 게임·외부 HWND에는 쓰지 않는다. 정책은 변경 금지 주석과 창모드·창모드 전체화면 회귀로 고정했고 전체 필수 게이트를 통과했다. 관리자 실제 HWND probe도 두 모드에서 foreground·게임 불변·승격/강등·외부 우선·스택 복구를 모두 확인했다. 사용자가 보고한 A/B/다른 모니터 이동 실게임 시나리오도 현재 실행 모드에서 정상이다. 실제 게임의 두 모드 전체 조합, 자동 생성 창·입력 도구 전체 및 30~60분 무조작 소크는 대기다.
