...
- `b1fbef7`의 지속형 모의 Drive + 실제 `main.js` 검사에서 손상 설정과 정상 숙제를 파일별로 분리해 숙제만 복원했다. 이어진 `needs-confirmation` 재시작에서 사용자 선택 전 파생 변경이 원격을 읽고 덮어쓰는 결함을 재현해 자동 전송을 대기시키도록 수정했으며, 이후 2.2초 동안 다운로드·업로드 0회와 로컬/원격 설정값 보존을 확인했다.
